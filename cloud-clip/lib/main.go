...
var UseEmbeddedStr = "true"   // 默认为 "false"
...
func (s *ClipboardServer) handle_push(w http.ResponseWriter, r *http.Request) {
	...
	// 使用 r.RemoteAddr 作为哈希的 IP 和端口部分，类似于 ws_send_devices
	// hash_murmur3 函数需要可用 (例如，来自 random.go 或工具包)
	// 假设 hash_murmur3 存在且工作方式与 ws.go 中类似
	...
	s.room_ws[conn] = room
	s.deviceConnected[deviceID] = deviceMeta // 存储完整的 DeviceMeta
	s.connDeviceIDMap[conn] = deviceID
	s.logger.Printf("新 WebSocket 客户端连接: %s (ID: %s), 房间: %s. 当前连接数: %d, 设备数: %d", conn.RemoteAddr(), deviceID, room, len(s.websockets), len(s.deviceConnected))

	// 向新客户端发送房间内当前连接的设备列表
	var devicesInRoom []DeviceMeta
	// getDeviceIDsInRoomLocked 需要在 runMutex 锁定时调用
	// 注意：此处的 getDeviceIDsInRoomLocked 实现可能需要改进以准确获取同房间设备
	for _, existingDeviceID := range s.getDeviceIDsInRoomLocked(room, "") { // deviceID 被排除
		if devMeta, ok := s.deviceConnected[existingDeviceID]; ok {
			devicesInRoom = append(devicesInRoom, devMeta)
		}
	}
	s.runMutex.Unlock() // 在可能长时间运行的发送操作之前解锁

	for _, devMeta := range devicesInRoom {
		wsMsg := WebSocketMessage{
			Event: "connect", // 前端期望的事件名
			Data:  devMeta,   // 直接发送 DeviceMeta
		}
	...
	newDeviceClientMsg := WebSocketMessage{
		Event: "connect",  // 前端期望的事件名
		Data:  deviceMeta, // 直接发送 DeviceMeta
	}
	s.broadcastWebSocketMessageToRoomExcept(newDeviceClientMsg, room, conn) // 使用新的广播函数

	// 发送历史消息 (现有逻辑)
	s.messageQueue.Lock()
	historyMessages := make([]PostEvent, 0, len(s.messageQueue.List)) // 使用大写 L
	for _, msg := range s.messageQueue.List {                         // 使用大写 L
	...
	for _, msg := range historyMessages { // historyMessages 是 []PostEvent
	...
	clientConfigData := struct {
		...
		File struct { // [!code ++]
			Expire int `json:"expire"` // [!code ++]
			Chunk  int `json:"chunk"`  // [!code ++]
			Limit  int `json:"limit"`  // [!code ++]
		} `json:"file"` // [!code ++]
	...
	go func() {
		defer s.cleanupWebSocketConnection(conn, deviceID, room) // 集中清理
...
func (s *ClipboardServer) cleanupWebSocketConnection(conn *websocket.Conn, deviceID string, room string) {
	...
	// 仅当关联了有效的 deviceID 时才从 deviceConnected 中删除
	if deviceID != "" {
		delete(s.deviceConnected, deviceID)
		s.logger.Printf("WebSocket 客户端断开连接: %s (ID: %s), 房间: %s. 当前连接数: %d, 设备数: %d", conn.RemoteAddr(), deviceID, room, len(s.websockets), len(s.deviceConnected))

		// 向房间广播设备断开连接事件
		disconnectWsMsg := WebSocketMessage{
			Event: "disconnect",                      // 前端期望的事件名
			Data:  map[string]string{"id": deviceID}, // 前端期望的载荷
		}
		s.runMutex.Unlock()
		s.broadcastWebSocketMessage(disconnectWsMsg, room) // 使用新的广播函数
	} else {
		s.logger.Printf("WebSocket 客户端断开连接 (无有效DeviceID): %s, 房间: %s. 当前连接数: %d", conn.RemoteAddr(), room, len(s.websockets))
		s.runMutex.Unlock()
	}
}
...
func (s *ClipboardServer) broadcastMessageToRoomExcept(message PostEvent, room string, exceptConn *websocket.Conn) {
	...
	for client, clientRoom := range s.room_ws {
		if client == exceptConn { // 跳过被排除的连接
			continue
		}
		if room == "" || clientRoom == room { // 匹配房间或全局广播
			err := client.WriteJSON(message)
			if err != nil {
				s.logger.Printf("错误: 写入消息到 WebSocket 客户端 %s 失败: %v。计划移除客户端。", client.RemoteAddr(), err)
				// 安排移除或直接处理，注意并发映射修改
				// 为简单起见，此处直接移除，但可以考虑一个单独的清理 goroutine
				client.Close()
				delete(s.websockets, client)
				delete(s.room_ws, client)
				// 如果此客户端有关联的 deviceID，也需要从 s.deviceConnected 中查找并移除
				// 这需要一个从 conn 到 deviceID 的映射。
			}
		}
	}
}
...
func (s *ClipboardServer) broadcastMessage(message PostEvent, room string) {
	s.logger.Printf("广播消息 (ID: %d, 类型: %s) 到房间 '%s'", message.Data.ID(), message.Event, room) // [!code word]
	s.runMutex.Lock()                                                                         // 使用 runMutex 保护 websockets 和 room_ws 的并发访问
	defer s.runMutex.Unlock()

	for client, clientRoom := range s.room_ws {
		if room == "" || clientRoom == room { // 如果 room 为空，则广播给所有客户端；否则只广播给指定房间
			err := client.WriteJSON(message)
			if err != nil {
				...
				delete(s.websockets, client)
				delete(s.room_ws, client)
				delete(s.deviceConnected, client.RemoteAddr().String()) // 假设 RemoteAddr().String() 是唯一的设备标识符
			}
		}
	}
}
	...
	for client, clientRoom := range s.room_ws {
		if room == "" || clientRoom == room {
			if err := client.WriteJSON(message); err != nil {
				s.logger.Printf("错误: 写入 WebSocketMessage 到客户端 %s 失败: %v。移除客户端。", client.RemoteAddr(), err)
				client.Close()
				delete(s.websockets, client)
				delete(s.room_ws, client)
				// 考虑也从 deviceConnected 中移除，需要 conn -> deviceID 的映射
			}
		}
	}
}
