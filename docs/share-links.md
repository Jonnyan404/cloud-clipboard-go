...
全程不碰正在跑的 9501。**29 个场景**全绿（这套临时环境是仓库里唯一的端到端，
后来也顺手接管了不相关的东西 —— 终端模式布局、任务列表打勾落盘等，都挂在这里）：
...
| 11 合并后的分享图标 | 面板有二维码 + `/#/s?t=` 链接 + 2 个按钮；`qrIconCount=0`、`linkIconCount=0`；**链接主机名必须是前端 origin**（不是后端） |
...
| 16a~16e 五个模式的分享入口 | 逐个模式打开详情弹窗 → 按钮文案是「分享链接」（不是旧的「复制链接」）→ 点开确认后弹出面板，且链接是 `/#/s?t=`；16e 另断言便签卡片操作行只剩 2 个按钮、**没有**链接图标 |
...
| 便签 | 阅读器弹窗动作行（`v-if="!isFile"`，**文本**条目）。卡片上原来那个链接图标**已删**，见下 |
...
返回 `{ raw, page }`（现在只有工作台还要用 `page`），没跟着改签名。

**便签卡片上那个「复制链接」图标（`mdi-link-variant`）删掉了。** 它只复制、不弹面板 ——
同一个分享动作在同一个模式里因此有两种行为（卡片上静默复制、阅读器里弹面板）。
收口到后者：卡片操作行只剩「复制/下载」和「删除」两个按钮。跟着下线的还有 `copyLink()`
和只被它用的 `contentUrl`。
...
`POST /share` 回的 `url` 是服务端用**请求的 Host** 拼的（`buildSharePageURL`）。只要中间有一层
会改写 Host 的代理，主机就是错的：

- **dev**：`web-vue3/vite.config.js` 的 proxy 每条都写了 `changeOrigin: true`，包括 `/share`。
  于是 Host 被改写成 `localhost:9501`（后端），拼出来的链接指向**后端** ——
  而 dev 的后端通常是**不带 `-tags embed` 编的**，根本没有前端（`GET /` → 404），点开就是白页。
- **线上**：任何 `proxy_set_header Host ...` 改写过的反代同理。

分享页是**前端路由**，必须落在前端所在的 origin 上。所以网页端不再采用服务端那个主机：
`util.js` 的 `withCurrentOrigin(url)` 把它换成本浏览器的 origin，**路径与 `#` 片段照原样保留**
（前缀是服务端决定的，所以带 prefix 部署也不会丢）。服务端那个 `url` 保持原样 ——
脚本 / 第三方客户端仍然可以直接用。

⚠️ **回归守卫**：e2e 场景 11 现在同时断言 `linkHostIsFrontend: true` 和 `linkHostIsBackend: false`。
以前只断言「链接里有 `/#/s?t=`」，所以这个 bug 一直没被抓到 —— 路径对、主机错，照样是白页。
