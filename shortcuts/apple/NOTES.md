...
**卡在"发"** —— 这才是 `getText` 之后真正的障碍：
Shortcuts 把**文本值**当 `File` body 上传时会把它物化成文件，若物化出的名字非法就报
**`未能存储该项目，因为文件名称无效`**，且**该请求根本不会发出**。
而当前剪贴板路径发的是**原始条目**（自带合法名字 `Clipboard <日期>`），所以一直很稳。

→ **未决**：需要验证"把 `getText` 的结果直接发到 `/text`"是否可靠。
→ 若可靠，`/upload/raw` 可以退休；若不可靠，则保留（它的价值是"客户端发条目、服务端读内容"）。
...
- **`/upload/raw`** —— 给**剪贴板路径**用。客户端**读不到剪贴板的文字**（`getText` 对中文返回乱码、
  有时直接抛异常），所以只能把原始字节交给服务端读内容并按魔数分流。详见
  [为什么剪贴板路径必须留在服务端](#为什么剪贴板路径必须留在服务端2026-09-17-实测)。
