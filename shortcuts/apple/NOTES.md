...
| 「剪贴板路径也能本地分流」 | **错（做不到）**。分类 ✅、读取 ✅ 都没问题，但客户端读到的文本是 **UTF-16 字节**，而客户端没有转 UTF-8 的动作——只能靠服务端 `/upload/raw` 的 `normalizeText` 解码。见下节 |

> **通用教训**：用"分步上报"定位失败时，**必须检查错误信息本身**——
> 否则会把"上报失败"误读成"被测动作失败"。
> 同理：**同一个动作在不同输入路径上的行为可能完全不同**，别把一条路径的结论推广到另一条。

### 剪贴板路径为什么必须留在服务端：卡在「编码」
...
**结论：`/upload/raw` 必须保留** —— 它承担的不是"读内容"，而是**编码转换**（UTF-16/32 → UTF-8）。
「全部本地实现」在这条上做不到。
