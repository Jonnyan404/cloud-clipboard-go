...
| 曾经的结论 | 实际情况 |
| :--- | :--- |
| 「`getText` 对剪贴板文本抛异常」 | **错**。把「第二步上报未到达」读成了动作抛异常，而 Shortcuts 报的错误指向上传环节。用固定标记解耦后，四次运行标记全部到达——`getText` **从未抛异常** |
| 「`getText` 不可靠是 `pbcopy` 写坏剪贴板的假象」 | **该推测被证伪**：剪贴板用 AppleScript 正确写入后依然复现。但**真正的失败原因至今未定论** |
