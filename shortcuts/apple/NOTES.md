...
| 剪贴板内容 | `getText` 的结果 |
| :--- | :--- |
| 中文文本 | 返回**乱码**（`中文文本内容` → `-N‡e‡e,g…Q¹[`） |
| ASCII 文本 | 常在上报环节失败（`未能存储该项目，因为文件名称无效`） |
| （另一次） | 直接抛 `无法将文本转换为 NSString` |

→ 客户端**既读不到文字、也就剥不了 HTML**，只能把**原始条目**发出去。
→ 所以 `/upload/raw` 的职责（读内容 + 分流 + 富文本降级）**无法搬到客户端**。

> 注：`getText` 在**分享路径**上是正常的（能返回文件内容 / 字符串本身）。
> 不可靠是**剪贴板路径专有**的。
