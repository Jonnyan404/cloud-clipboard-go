...
- **分享路径**：**先看扩展名，再用名字启发式兜底**（2026-09-17 定稿）
...
- **扩展名怎么来的**：`is.workflow.actions.properties.files`（"获取文件详情 → 扩展名"）。
  这个动作 cherri 的动作表里没有，只能用 `rawAction` 调；且它**没有输入参数**、吃「上一个动作的输出」，
  实测必须**显式给 `WFInput`** 才拿得到值 —— 由 `source/patch_shortcut.py` 在构建后补上。

- **误判安全**：图片/二进制即使被判成文本，服务端仍按魔数嗅探重新分流

**验收矩阵（2026-09-17，9/9 全绿）**：
...
> 第 4 条是这轮的重点：`.md` 曾被误判成文本（markdown 的首个标题=文件名，骗过了名字启发式），
> 靠扩展名判据修复。第 7 条同时说明**文件名也保住了**（此前是 `clipboard.png`）。
