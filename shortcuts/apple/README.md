...
**更关键的是 `getText` 对剪贴板文本条目会抛异常。** 用诊断捷径（分两步上报 `typeOf` 与 `getText` 的结果，只收到第一步即说明 `getText` 抛异常）实测：

| 输入 | `typeOf` | `getText` 结果 |
| :--- | :--- | :--- |
| 纯 ASCII 剪贴板 | `文本` | **抛异常**（复测仍失败，可复现） |
| 中文剪贴板 | `文本` | **抛异常** |
| HTML / 富文本剪贴板 | `多信息文本` | ✅ 成功，且正确剥成纯文本 |
| **URL scheme 文本输入** | `文本` | ✅ **成功** |
| `-i` `.txt` 文件（含中文） | `文本` | ✅ 成功，中文完好 |
| `-i` `.png` | `图像` | ⚠️ "成功"但只返回文件名 `test`，无意义 |
| `-i` `.html` 文件 | `多信息文本` | ⚠️ 成功但**乱码**（UTF-8 被按错误编码解码） |

→ **失败是特例性的：只有"剪贴板里的纯文本条目"抛异常**，文本输入与文件输入都正常。
→ 因此客户端无法用一条无分支的路径处理剪贴板文本；要用它就得先判断类型，而判断类型又只能靠 `typeOf`（本地化）——绕回原问题。
→ 另注：`getText` 可用与否**不能**用来区分"文件 vs 文本"（两者都可用，且 `typeOf` 都是 `文本`）。
...
> 订正记录：曾推测「`getText` 不可靠」是 `pbcopy` 写坏剪贴板造成的假象。**该推测已被上表证伪**——剪贴板用 AppleScript 正确写入后依然失败。前人对剪贴板路径的判断是准确的；但"不可靠"是特例性的，别推广到所有输入路径。
