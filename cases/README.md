...
这个目录放**不属于任何一种语言**的契约数据。它入库，因为 Rust 侧的契约测试读它。

⚠️★ **2026-09-25：Rust 实现已拆成独立仓库**（`clip9`，与这个仓库平级），
那份 `cases/` 是它的**冻结输入**。**这里这份的角色变了** ——
它是 **Go 自己测试的校验基准**：非更新模式下 `*_fixture_test.go` 会读回文件逐字比，
不一致就 `t.Fatalf("…**这是一次契约变更**，不是测试坏了")`。

⚠️ **两份会漂，而漂是信号**（契约变了、Rust 那边该跟），所以别让它悄悄发生：

```bash
diff -rq ../clip9/cases cases        # 两个仓库平级时
# 或者跑 clip9 那边的一节专门检查：cd ../clip9 && node tools/compare-with-go.mjs
```
